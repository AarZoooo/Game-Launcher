...
const seedGames: Game[] = [
  {
    ...
    cover: '/mock/sekiro-card.png',
    hero: '/mock/home-hero.png',
    accent: 'gold',
    featured: true,
    inLibrary: true,
  ...
  },
  {
    ...
    coop: 'Yes',
    completion: 'Sandbox',
    cover: '/mock/satisfactory-card.png',
    accent: 'silver',
    inLibrary: true,
  ...
  },
  {
    ...
    coop: 'Yes',
    completion: 'Single player',
    cover: '/mock/stardew-card.png',
    hero: '/mock/stardew-hero.png',
    accent: 'gold',
    inLibrary: true,
    genreLabel: 'RPG • 8.2',
    path: 'C:\\Windows\\System32\\notepad.exe',
    similarIds: ['coral-island', 'sun-haven', 'sandrock', 'roots-of-pacha', 'terraria']
  },
  {
    id: 'ghost-yotei',
    ...
    coop: 'No',
    completion: 'Single player',
    cover: '/mock/ghost-tsushima-card.png',
    hero: '/mock/ghost-hero.png',
    accent: 'olive',
  ...
  },
  {
    ...
    coop: 'No',
    completion: 'Single player',
    cover: '/mock/hollow-knight-card.png',
    accent: 'silver',
    inRecommendations: true,
    genreLabel: 'Action • 9.2'
  },
  {
    ...
    coop: 'Yes',
    completion: 'Single player',
    cover: '/mock/cyberpunk-card.png',
    hero: '/mock/cyberpunk-hero.png',
    accent: 'silver',
    inRecommendations: true,
    genreLabel: 'Open World • 7.9',
    similarIds: ['deus-ex', 'the-ascent', 'ghostrunner-2', 'observer', 'cloudpunk']
  },
  {
    id: 'uncharted-4',
    title: 'Uncharted 4',
  ...
  },
  {
    ...
    coop: 'Yes',
    completion: 'Competitive',
    cover: '/mock/battlefield-card.png',
    accent: 'silver',
    inRecommendations: true,
  ...
  },
  {
    ...
    coop: 'Yes',
    completion: 'Co-op',
    cover: '/mock/it-takes-two-card.png',
    accent: 'gold',
    inRecommendations: true,
  ...
  },
  {
    ...
    coop: 'Yes',
    completion: 'Multiplayer',
    cover: '/mock/sea-of-thieves-card.png',
    accent: 'gold',
    inRecommendations: true,
  ...
  },
  {
    ...
    coop: 'No',
    completion: 'Story',
    cover: '/mock/roots-pacha-card.png',
    accent: 'gold',
  ...
  },
  {
    ...
    coop: 'No',
    completion: 'Single player',
    cover: '/mock/coral-island-card.png',
    accent: 'gold',
  ...
  },
  {
    ...
    coop: 'No',
    completion: 'Single player',
    cover: '/mock/ghostrunner-card.png',
    accent: 'silver',
    genreLabel: 'Action • 8.0'
  },
  {
...
function normalizeGame(game: Game, index: number): Game {
  ...
  const defaultStatus: GameStatus =
    parseHours(game.hours) > 50 ? 'played' : parseHours(game.hours) > 0 ? 'playing' : 'want';
  ...
  return {
    ...
    tags: game.tags || [],
    resumeState: game.resumeState || (parseHours(game.hours) > 0 ? 'resume' : 'restart'),
...
export const homeRecentIds = ['sekiro', 'cs2', 'satisfactory', 'hades', 'gta-vi'];
export const homeExploreIds = ['elden-ring', 'rdr2', 'hollow-knight', 'cyberpunk-2077', 'uncharted-4'];
export const libraryOrderIds = ['sekiro', 'cs2', 'satisfactory', 'hades', 'gta-vi', 'rust', 'stardew'];
  ...
  return normalizeGame(
    {
      ...
      coop: 'Unknown',
      completion: 'Unknown',
      cover: game.coverArt || '/mock/elden-card.png',
      hero: game.coverArt || '/mock/elden-card.png',
      accent,
      path: game.exePath,
...
function toStoredGame(game: Game): StoredGame {
  return {
    ...
    coverArt: game.cover,
    platform:
      game.platformType && game.platformType !== 'suggested' && game.platformType !== 'wishlist'
        ? game.platformType
    ...
    genres: game.storageGenres?.length ? game.storageGenres : splitGenres(game.genres),
...
function mergeStoredLibrary(storedGames: StoredGame[]) {
  ...
  return [...libraryGames, ...fallbackGames.filter((game) => !libraryIds.has(game.id.toLowerCase()))];
}
...
function createGameStore() {
  ...
  return {
    ...
    toggleFavorite: (id: string) =>
      update((items) =>
        items.map((game) => (game.id === id ? { ...game, favorite: !game.favorite } : game))
      ),
        ...
        items.map((game) =>
          game.id === id
            ? {
                ...
                syncStatus: !game.cloudSyncEnabled ? 'Sync enabled' : 'Sync paused'
              }
            : game
        )
      ),
    setLaunchOptions: (id: string, launchOptions: string) =>
      update((items) =>
        items.map((game) => (game.id === id ? { ...game, launchOptions } : game))
      ),
    updateDetails: (
      id: string,
      payload: Partial<Pick<Game, 'title' | 'path' | 'tags' | 'cover' | 'savePath'>>
    ) =>
      updateAndPersist((items) =>
        items.map((game) =>
          game.id === id
    ...
    addImportedGames: (imports: ImportedGameResult[]) =>
      updateAndPersist((items) => {
        const seen = new Set(
          items.flatMap((game) => [game.id.toLowerCase(), (game.path || '').toLowerCase()])
        );
...
export const favoriteGames = derived(games, ($games) => $games.filter((game) => game.favorite));
export const continuePlayingGames = derived(games, ($games) =>
  $games.filter((game) => !game.hiddenFromContinue && parseHours(game.hours) > 0).slice(0, 5)
);
